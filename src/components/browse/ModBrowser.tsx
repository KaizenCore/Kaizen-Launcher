...
export function ModBrowser() {
  ...
  useEffect(() => {
    const loadInstances = async () => {
      try {
        console.log("[ModBrowser] Loading instances...")
        const result = await invoke<Instance[]>("get_instances")
        // Filter to only show client instances with mod loaders
        const modInstances = result.filter(
          (i) => !i.is_server && i.loader && ["fabric", "forge", "neoforge", "quilt"].includes(i.loader.toLowerCase())
        )
        setInstances(modInstances)
        console.log("[ModBrowser] Loaded", modInstances.length, "mod-capable instances")
      } catch (error) {
        console.error("[ModBrowser] Failed to load instances:", error)
      }
    }
    loadInstances()
  }, [])
    ...
    const searchMods = async () => {
      ...
      try {
        ...
        const response = await invoke<ModSearchResponse>("search_modrinth_mods", {
          query: debouncedQuery,
          gameVersion: selectedInstance?.mc_version || null,
  ...
  }, [debouncedQuery, selectedInstance?.mc_version, selectedInstance?.loader, selectedLoader, categoriesKey, sortBy, currentPage])
  ...
  }, [selectedMod, selectedVersion, selectedInstance, t])
